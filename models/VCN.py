...
class VCN(nn.Module):
    ...
    def forward(self,im,disc_aux=None):
        ...
        ## matching 2
        up_flow3 = F.upsample(flow3, [im.size()[2]//4,im.size()[3]//4], mode='bilinear', align_corners=True)*2
        ...
        # append coarse hypotheses
        flow2h = torch.cat((flow2h, F.upsample(flow3h.detach()*2, [flow2h.shape[2],flow2h.shape[3]], mode='bilinear', align_corners=True)),1)
        ent2h = torch.cat((ent2h, F.upsample(ent3h, [flow2h.shape[2],flow2h.shape[3]], mode='bilinear', align_corners=True)),1)
        ...
        flow2 = F.upsample(flow2, [im.size()[2],im.size()[3]], mode='bilinear', align_corners=True)
        ...
        if self.training:
            ...
            up_flow3 = F.upsample(up_flow3, [im.size()[2],im.size()[3]], mode='bilinear', align_corners=True)*4
            ...
            oor2 = F.upsample(oor2[:,np.newaxis], [im.size()[2],im.size()[3]], mode='bilinear', align_corners=True)[:,0]
